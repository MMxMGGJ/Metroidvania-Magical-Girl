# Note: PlayerCharacterBase and related classes were copied over from komehara's Super Sonic Heroes
...
func deferred_setup():
	...
	for child in states_parent.get_children():
		...
		if not state:
			push_error("[PlayerCharacterBase] child '%s' is under states_parent, yet not a PlayerCharacterState" % child.get_path())
			continue
...
func get_state_by_name(state_name: StringName) -> PlayerCharacterState:
	...
	if not state:
		push_error("[PlayerCharacterBase] %s: get_state_by_name: state_name '%s' is not in states_dict keys" %
			[name, state_name])
...
func set_next_state_by_name(next_state_name: StringName, allow_restart: bool = false) -> PlayerCharacterState:
	...
	if not found_next_state:
		push_error("[PlayerCharacterBase] %s: set_next_state_by_name: next_state_name '%s' " %
				[name, next_state_name],
			"is not in states_dict keys. Return null.")
		return null

	if current_state == found_next_state and not allow_restart:
		push_warning("[PlayerCharacterBase] %s: set_next_state_by_name: character is already in state '%s' " %
				[name, next_state_name],
...
func add_active_tag(tag: StringName):
	...
	if tag_index >= 0:
		push_warning("[PlayerCharacterBase] add_active_tag: tag '%s' is already active, " % tag,
...
func remove_active_tag(tag: StringName):
	var tag_index := active_tags.find(tag)
	if tag_index < 0:
		push_warning("[PlayerCharacterBase] remove_active_tag: could not find tag '%s'" % tag)
		return
...
func stop_invincibility_blink_tween():
	if invincibility_blink_tween:
		...
		push_error("[PlayerCharacterBase] stop_invincibility_blink_tween: invincibility_blink_tween is not set")
...
func spawn_fx_trail_and_start_tracking_self():
	...
	# for safety
	if current_fx_trail:
		push_warning("[PlayerCharacterBase] spawn_fx_trail_and_start_tracking_self: has already ",
	...
	if not fx_trail_prefab:
		push_error("[PlayerCharacterBase] spawn_fx_trail_and_start_tracking_self: no fx_trail_prefab assigned")
		return
	...
	if not fx_trail:
		push_error("[PlayerCharacterBase] spawn_fx_trail_and_start_tracking_self: fx_trail is null, ",
...
func stop_fx_trail_tracking_self():
	if not current_fx_trail:
		push_error("[PlayerCharacterBase] stop_fx_trail_tracking_self: current_fx_trail is null")
